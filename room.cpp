...
void Room::deliver(ParticipantPointer participant, Message &message){
    ...
    while (!messageQueue.empty()) {
        ...
        for (ParticipantPointer curr_participant : participants) {
            if (participant != curr_participant) {
                curr_participant->write(msg);
            }
        }
    }
}
void Session::handle_read(boost::system::error_code ec, std::size_t bytes_transferred) {
    if (!ec) {
